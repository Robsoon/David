...
function fetchAllScores($rows, $tfs, $g) {
	...
	if ($tfs == 'TFS_10') {
		...
		$data[10] = mysql_select_multi("SELECT id, name, vocation, Cast((Case length(description) when 0 then 0 else substring(description, 9, char_length(description)-9) end) as UNSIGNED) resets FROM players WHERE group_id < $g  order by 4 desc, experience desc limit 0, $rows;");
	} else {
		...
		$data[7]  = mysql_select_multi("SELECT `id`, `name`, `vocation`, `experience`, `level` AS `value` FROM `players` WHERE `group_id` < $g ORDER BY `experience` DESC limit 0, $rows;");
		$data[8]  = mysql_select_multi("SELECT `id`, `name`, `vocation`, `maglevel` AS `value` FROM `players` WHERE `group_id` < $g ORDER BY `maglevel` DESC limit 0, $rows;");
		$data[10] = mysql_select_multi("SELECT id, name, vocation, Cast((Case length(description) when 0 then 0 else substring(description, 9, char_length(description)-9) end) as UNSIGNED) as value FROM players WHERE group_id < $g  order by 4 desc, experience desc limit 0, $rows;");
